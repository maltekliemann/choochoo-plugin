...
```bash
{{INVOKE:spec}} [source-file] [spec-name]
```

- From plan file: `{{INVOKE:spec}} plans/my-feature.md`
- From conversation: `{{INVOKE:spec}}` (uses context)
- With explicit name: `{{INVOKE:spec}} plans/feature.md auth-system`
...
1. **Add feedback** in `<review>` tags:
   ...
   ```
2. **Run `{{INVOKE:spec}}` again** - AI processes feedback
...
```bash
{{INVOKE:pour}} [target-tasks] [spec-file] [formula]
```

Examples:
- Auto-detect everything: `{{INVOKE:pour}}`
- Target 80 tasks: `{{INVOKE:pour}} 80`
- Specific spec and formula: `{{INVOKE:pour}} 80 auth-system choochoo`
...
| Command | Purpose |
|---------|---------|
| `{{INVOKE:install}}` | Set up Ralph in project |
| `{{INVOKE:spec}}` | Generate/refine spec from plan |
| `{{INVOKE:pour}}` | Create beads from spec |
