...
- **Git worktrees** for agent isolation — each molecule runs in its own worktree, separate from the main checkout
...
A single linear chain of molecules linked by `blocks` dependencies:

```
mol-A → mol-B → mol-C → mol-D → ...
```

Each molecule blocks the next. Only one molecule is unblocked at any point — this
is the compound's _cursor_. The runner finishes mol-A (work → inspect → merge),
then advances the cursor to mol-B, and so on. If any molecule fails, the compound
stops. Non-linear compounds (one molecule blocking multiple) are rejected.
...
Only use CI commands — test suites, linters, type checkers, build steps. Do NOT
use file-existence checks (`test -f`), grep, or anything outside of running CI.
...
Chain molecules into a single linear compound with `blocks` dependencies:

```bash
# Chain: mol-1 → mol-2 → mol-3 (each blocks the next)
bd dep add <mol-2> <mol-1> --type blocks   # mol-2 waits for mol-1
bd dep add <mol-3> <mol-2> --type blocks   # mol-3 waits for mol-2
```

`bd ready` only surfaces the first unblocked molecule (the compound cursor).
When mol-1 closes, mol-2 unblocks and becomes the cursor.
...
All backends are launched in dangerous/YOLO permissions mode by default. This is
the one setting that Choo! Choo! overrides — the agent runs fully autonomous with
no interactive permission prompts:

- **Claude Code**: `--dangerously-skip-permissions`
- **Cursor**: `--force`
- **Codex**: `codex exec` (full-auto by default)

The worktree provides isolation — the agent can only damage its own copy.
...
3. Fail → reopen steps closed this session, inject the exact verification error
   (command, exit code, stdout, stderr) into the root bead's comments, then retry
   with a fresh agent. The next agent should read these comments to understand what
   went wrong.
...
| `bd comments <id> --json` | List comments on a bead (includes verification errors) |
| `bd comments add <id> "text"` | Add a comment to a bead |
